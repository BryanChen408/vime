...
# PPO variant of run-qwen3-30B-A3B-npu.sh.
#
# --advantage-estimator ppo adds a critic: the same backbone with a value head, initialised
# from --ref-load. Training then forces actor offload so the two models take turns on the
# same cards, and the critic writes its checkpoints to <--save>_critic.
#
# On a larger model both optimizers on one host can exhaust its memory. Give the critic its
# own cards with --resource-layout in that case; see scripts/resource_layout.example.yaml.
...
PPO_ARGS=(
   ...
   # Turns on the critic: same backbone with a value head, initialised from --ref-load.
   --num-critic-only-steps 8
   # Warm the value head before the actor starts moving, or early advantages are noise.
   --value-clip 0.2
   --gamma 1.0
   --lambd 1.0
   --kl-coef 0
