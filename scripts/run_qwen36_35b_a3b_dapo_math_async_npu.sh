#!/bin/bash
# Qwen3.6-35B-A3B (GDN/MoE) DAPO-math RL — ASYNC, single node, NPU.
...
RUN_ID=${RUN_ID:-qwen36_dapo_math_async_$(date +%Y%m%d-%H%M%S)}
...
RUN_ROOT=${RUN_ROOT:-${VIME_ROOT}/runs/dapo_math_async_$(date +%Y%m%d_%H%M%S)}
