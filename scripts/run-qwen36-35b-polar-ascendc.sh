#!/bin/bash
# vime + polar 【AscendC / rollout-only 纯推理】算子 RL 启动(qwen3.6-35B-A3B / NPU)。
# 派生自 run-qwen36-35b-polar-minimal.sh(triton);AscendC 专属 delta 5 处(见 [ASCENDC] 标注):
#   ① 数据集 → npukernelbench_level1_ascendc  ② context 262144(rollout+vllm 两处)
#   ③ POLAR_ANTHROPIC_DEFAULT_MAX_TOKENS=12288(基线已默认)  ④ FEAT_LB_PROXY=1(多引擎必开)
#   ⑤ --num-gpus-per-node(端口分配器按每节点卡数算引擎数)。backend 切换在 polar profile.t2a,不在此。
# 前置:宿主机先用 profile.t2a.yaml 起 polar,其推理端点指向 vime vllm 的 :${VLLM_ROUTER_PORT}。
# 卡位:polar 验证 0-3;后 12 卡(4-15)纯推理(3 engine × 4,TP4)。
set -ex
...
ACTOR_NUM_GPUS_PER_NODE=${ACTOR_NUM_GPUS_PER_NODE:-8}    # rollout-only 下被忽略(actor gpus=0)
ROLLOUT_NUM_GPUS=${ROLLOUT_NUM_GPUS:-12}                 # [ASCENDC] 12 卡纯推理(4-15,3 engine × 4)
ROLLOUT_NUM_GPUS_PER_ENGINE=${ROLLOUT_NUM_GPUS_PER_ENGINE:-4}

# [ASCENDC ④] 多引擎(>1)必开 LB proxy:Rust router 丢 return_token_ids → Trace 校验整批废。放数组外。
FEAT_LB_PROXY=${FEAT_LB_PROXY:-1}
# [ASCENDC ⑥ 纯推理] rollout-only:不训练,actor gpus=0,全部卡给 rollout。
DEBUG_ROLLOUT_ONLY=${DEBUG_ROLLOUT_ONLY:-1}

# ─── polar 数据 / 端点 ───
POLAR_OUTPUT_DIR=${POLAR_OUTPUT_DIR:-output/polar_bridge}
OPERATOR_DATA_ROOT=${OPERATOR_DATA_ROOT:-/home/docker/datasets/op_tasks/npukernelbench_level1_ascendc}  # [ASCENDC ①] AscendC 数据集(triton 集里无同名 .json → docker cp 退1 → session init 全挂)
...
TOPO_ARGS=(
   ...
   --num-gpus-per-node ${NPUS_PER_NODE}            # [ASCENDC ⑤] 端口分配器按每节点卡数算引擎数(12//4=3);默认 8 会把第 3 引擎误判为第二台机 → 15000 撞车
)
# [ASCENDC ⑥] rollout-only 纯推理:num_actor_gpus=0,全部卡给 rollout。
[ "${DEBUG_ROLLOUT_ONLY:-0}" = "1" ] && TOPO_ARGS+=(--debug-rollout-only)
...
ROLLOUT_ARGS=(
   ...
   --rollout-max-context-len "${ROLLOUT_MAX_CONTEXT_LEN:-262144}"   # [ASCENDC ②] AscendC 会话长(prompt p50 ~114k);131072 时 http_400 死一片
...
VLLM_ARGS=(
   ...
   --vllm-max-model-len "${VLLM_MAX_MODEL_LEN:-262144}"   # [ASCENDC ②] 两处 context 都要 262144
