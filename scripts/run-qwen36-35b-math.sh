#!/bin/bash
# vime + polar MATH RL 启动(qwen3.6-35B-A3B / NPU)—— run-qwen36-35b-polar-minimal.sh 的 math 派生:
#   唯一差异 = 删掉 `--operator-tasks-dir`(math 无算子任务文件,不 attach task_source)。
#   由 scripts/start_math.sh 调用(它注入 math 数据/规模/端点 env);勿手工直接跑。
# 前置:宿主机先用 profile.math.yaml 起 polar,其推理端点指向 vime vllm LB proxy 的 :${VLLM_ROUTER_PORT}。
