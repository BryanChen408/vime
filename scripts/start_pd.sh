...
CURRENT_IP=80.5.25.141  MASTER_ADDR=80.5.25.141  NNODES=2  NPUS_PER_NODE=16  SOCKET_IFNAME=enp48s3u1u1 \
ACTOR_NUM_GPUS_PER_NODE=16 \
ROLLOUT_NODE_IP=80.5.25.140 \
ROLLOUT_NUM_GPUS=8 \
FEAT_PD_DISAGG=1 \
VLLM_PD_CONFIG=/workspace/vime/scripts/vllm_qwen36_35b_polar_dual140_pd_8card.yaml \
...
VIME_MEM_PROBE=1 \
no_proxy=127.0.0.1,localhost,80.5.25.141,80.5.25.140,.huawei.com,local,.local \
NO_PROXY=127.0.0.1,localhost,80.5.25.141,80.5.25.140,.huawei.com,local,.local \
TP=2 \
POLAR_TRAJECTORY_PG_FLOOR=0.05 \
POLAR_ROLLOUT_URL=http://80.5.25.140:8180 \
...
HCCL_BUFFSIZE=512 \
ROLLOUT_BATCH_SIZE=4  N_SAMPLES_PER_PROMPT=8  GLOBAL_BATCH_SIZE=16  NUM_ROLLOUT=100 \
