...
TRACKING_ARGS=()
# [vime fix] wandb disabled - no API key
# if [ "${USE_WANDB}" = "1" ]; then
...
TOPO_ARGS=(
   ...
   # 再用它算 node_index。不传则停在默认 8 → 8//4=2 → 单台 16 卡机被劈成两个虚构节点 →
   # decode 组 engine 3 落到 node_index=1,游标回退 base_port,与 engine 1 撞同一段
   # bootstrap 区间(实测 15076-15079)→ Mooncake handshake 元数据串台 → transfer ret:-1。
   # debug_rollout_only 走不到 --resource-layout 的同源修正(arguments.py:1809 禁止共用),必须显式传。
...
#
# 扫描区间和 Mooncake KV 握手端口重叠: vime 每个 engine 分配
# port / port+1(nccl) / port+2..port+1+tp(bootstrap)。往 bootstrap 端口发
# GET /metrics 会让 Mooncake 把 HTTP 头当二进制长度前缀读,在 vllm 日志里刷
# "readString: too large length from socket: 8387229930220700999"(小端即 'GET /met')。
# 故告知面板 engine 内部端口窗口宽度 = 1(nccl) + tp,发现 engine 后自动隔离。
