#!/bin/bash
# Qwen3.6-35B-A3B against the polar operator environment on NPU, with a critic.
#
# Forked from run-qwen36-35b-polar-ppo.sh, adding the options that keep the value function
# usable on long, multi-turn trajectories. See scripts/run-qwen3-30B-A3B-sao-npu.sh for the
# same configuration without this host's paths baked in.
#
# The value head trains alone first, the critic then updates twice per policy update and only
# over its experts and value head, tool output stays out of the GAE recursion, and the credit
# horizon follows the response length. Clipping is wide and asymmetric, which suits the code
# rewards here; maths rewards want something closer to 0.3/5.0.
#
# The reward-side KL has to stay at zero: the critic is built without a reference model.
#
# Rollout traces here reach 22-40K tokens and the adapter drops anything above
# max_tokens_per_gpu * cp_size, so MAX_TOKENS_PER_GPU has to be raised or every long trace is
# silently discarded and the run makes no progress.
...
# ─── Critic setup, plus the options that keep it usable on long trajectories ───
PPO_ARGS=(
   --advantage-estimator ppo
   # Adds the critic: same backbone with a value head, initialised from --ref-load. Actor and
   # critic then take turns on the same cards through CPU offload.
   --num-critic-only-steps ${NUM_CRITIC_ONLY_STEPS:-10}
   # The value head trains alone first; without it the early advantages are noise.
   --value-clip 0.2
   --gamma 1.0
   # Superseded per sample by --gae-lambda-alpha below.
   --lambd 1.0
   --kl-coef 0
   ...
   --entropy-coef 0.00
   --use-tis
   # Asymmetric and wide: a sparse code reward stalls under a tight upper clip. Maths rewards
   # want something closer to 0.3/5.0.
   --eps-clip ${EPS_CLIP:-0.8}
   --eps-clip-high ${EPS_CLIP_HIGH:-3.0}

   # Normalise the loss over every trainable token in the batch rather than per sequence.
   --calculate-per-token-loss

   --critic-update-steps ${CRITIC_UPDATE_STEPS:-2}

   # Only the routed experts and the value head train; attention, norms, embedding, router and
   # the shared expert stay frozen. Widen to 'mlp\. output_layer' to include the latter two.
   --critic-only-train-params-name-list mlp.experts output_layer

   # These belong together: skipping tool output only changes anything below lambda 1, and the
   # adaptive lambda is what puts it there.
   --skip-observation-gae
   --gae-lambda-alpha ${GAE_LAMBDA_ALPHA:-1.5}
)
...
echo "[sao] critic_warmup=${NUM_CRITIC_ONLY_STEPS:-10} critic_updates=${CRITIC_UPDATE_STEPS:-2} eps_clip=${EPS_CLIP:-0.8}/${EPS_CLIP_HIGH:-3.0} gae_lambda_alpha=${GAE_LAMBDA_ALPHA:-1.5} skip_obs=on frozen_critic=on"
...
if [ "$MASTER_ADDR" = "$CURRENT_IP" ]; then
   ...
   while true; do
      ...
      if [ "$active_node_count" -eq "$NNODES" ]; then
         ...
         # The critic usually wants a larger learning rate than the policy; --lr is shared, so
         # give it its own here. See scripts/sao_critic_config.yaml.
         [ -n "${CRITIC_CONFIG:-}" ] && EXTRA_ARGS+=(--megatron-config-path "${CRITIC_CONFIG}")
