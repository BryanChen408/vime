...
def build_prefill_request(req_data: dict) -> dict:
    ...
    # [PD-mooncake W3] prefill 只算 1 个 token。GenerateRequest(/inference/v1/generate)的
    #   max_tokens 在 sampling_params 里(非顶层);OpenAI(/v1/completions)是顶层。分别设。
    #   ⚠️ generate 不设成嵌套的话,P 会按客户端原 max_tokens 全量生成(prefill 失效)。
    if "token_ids" in payload and isinstance(payload.get("sampling_params"), dict):
        sp = dict(payload["sampling_params"])
        sp["max_tokens"] = 1
        sp["min_tokens"] = 1
...
# [PD-mooncake W3] vime rollout 端点(GenerateRequest{token_ids,sampling_params,
#   kv_transfer_params} → GenerateResponse{token_ids,kv_transfer_params},serve/disagg
#   原生支持 PD 往返)。复用 handle_completions_impl 的 assign(prefill on P)→ stream(decode on D)
#   编排;GenerateResponse 主路径原样透传(token_ids 原生),OpenAI 的 stop_reason 重算对
#   generate 为 None → retry 自然跳过(mooncake 传输硬失败即 raise,非软重算)。
@app.post("/inference/v1/generate")
@with_cancellation
async def handle_generate(request: Request):
    return await handle_completions_impl("/inference/v1/generate", request)
