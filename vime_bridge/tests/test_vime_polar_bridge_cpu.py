...
def test_trajectory_reducer_rejects_a_batch_without_keys():
    import pytest

    from vime_bridge.trajectory_loss import get_trajectory_pg_loss_reducer

    with pytest.raises(ValueError, match="trajectory keys"):
        get_trajectory_pg_loss_reducer(
            total_lengths=[4],
            response_lengths=[2],
            loss_masks=[[1, 1]],
...
def test_trajectory_reducer_rejects_a_batch_without_trace_counts():
    import pytest

    from vime_bridge.trajectory_loss import get_trajectory_pg_loss_reducer

    with pytest.raises(ValueError, match="trace counts"):
        get_trajectory_pg_loss_reducer(
            total_lengths=[4],
            response_lengths=[2],
            loss_masks=[[1, 1]],
            trajectory_keys=[("a", 0)],
            trajectory_trace_counts=None,
            batch={},
        )
