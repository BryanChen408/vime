...
def _build_sample(
    *,
    ...
    # [A: logprob-integrity consumer] polar 的 _logprob_integrity 检测 per-token misattribution / -9999
    #   sentinel(全 response token,含 CoT),记进 trace.metadata["logprob_integrity"](仅有问题时才有此键)。
    #   放开 CoT(POLAR_MASK_REASONING=0)后 CoT logprob 首次进训练,坏 logprob 会毒化 PPO/DIS 比值 → 这里拒之。
    #   有问题就 remove_sample(走 drain-backfill);VIME_KEEP_BAD_LOGPROB=1 可关(调试)。
    ...
    if integrity_reject:
        logger.warning(
            ...
            result.session_id, trace_index, _integrity,
        )
