...
def _build_sample(
    *,
    ...
    # The rollout server flags per-token misattribution and sentinel values on the response
    # log-probs, and only records the key when it found something. Those log-probs go straight
    # into the importance ratio, so a trace carrying them is dropped rather than trained on;
    # reasoning tokens make this worse, since they reach training only once they are unmasked.
    trace_metadata = getattr(trace, "metadata", None)
    integrity = trace_metadata.get("logprob_integrity") if isinstance(trace_metadata, dict) else None
    integrity_reject = bool(integrity)
    if integrity_reject:
        logger.warning(
            "Dropping trace %d from session %s: rollout log-probs failed their integrity check (%s)",
            trace_index,
            result.session_id,
            integrity,
        )
...
def _load_sample_type() -> Any:
    try:
        ...
        raise ImportError(
            "vime is required to convert Polar rollouts into training samples, and "
            "vime.utils.types could not be imported."
        ) from exc
    return Sample
