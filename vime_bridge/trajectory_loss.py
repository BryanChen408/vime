...
def _local_grouped_trajectory_reducer(
    total_lengths: list[int],
    response_lengths: list[int],
    loss_masks: list[torch.Tensor],
    trajectory_keys: Sequence[Any],
    *,
    ...
    normalized_keys = [_normalize_key(key, idx) for idx, key in enumerate(trajectory_keys)]

    def reduce_pg_loss(pg_loss: torch.Tensor) -> torch.Tensor:
        per_trace: list[torch.Tensor] = []
        for x_i, local_mask, global_mask in zip(
            pg_loss.split(split_lengths, dim=0),
            local_masks,
            loss_masks,
            strict=False,
        ):
            per_trace.append((x_i * local_mask).sum() / torch.clamp_min(global_mask.sum(), 1))

        grouped: "OrderedDict[tuple[Any, ...], list[torch.Tensor]]" = OrderedDict()
        for key, value in zip(normalized_keys, per_trace, strict=False):
            grouped.setdefault(key, []).append(value)

        return sum(torch.stack(values).mean() for values in grouped.values())
...
def _sample_mean_reducer(
    ...
    *,
    ...
    def reduce_pg_loss(pg_loss: torch.Tensor) -> torch.Tensor:
        return sum(
            (x_i * local_mask).sum() / torch.clamp_min(global_mask.sum(), 1)
            for x_i, local_mask, global_mask in zip(
                pg_loss.split(split_lengths, dim=0),
                local_masks,
                loss_masks,
