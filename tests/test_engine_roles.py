...
def test_weight_sync_split_uses_roles_for_partial_share():
    from vime.backends.megatron_utils.update_weight.update_weight_from_tensor import (
        _resolve_colocated_engine_count,
    )
    ...
    args = types.SimpleNamespace(
        ...
        rollout_num_gpus_per_engine=2,
        resource_layout_spec=types.SimpleNamespace(
            actor=[_Item("actor", list(range(16)))],
            rollout=[
                _Item("actor", list(range(8))),
                _Item("remote", list(range(8))),
            ],
            rollout_has_share=True,
        ),
    )
    ...
    # The old slot-range heuristic returned 8 because every rollout slot was
    # below actor_gpus=16. Physical roles correctly identify only four.
    assert _resolve_colocated_engine_count(args, offsets, [2] * 8) == 4


def test_weight_sync_actor_rank_map_uses_actor_placement():
    from vime.backends.megatron_utils.update_weight.update_weight_from_tensor import (
        _actor_ranks_by_gpu_slot,
    )

    assert _actor_ranks_by_gpu_slot(T5_PARTIAL_SHARE) == {
        0: (4, 5),
        2: (6, 7),
        4: (8, 9),
        6: (10, 11),
