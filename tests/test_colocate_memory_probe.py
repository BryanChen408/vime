...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("0", False),
        ("1", True),
        # Deliberately strict: all probes use the same documented switch.
        ("true", False),
        ("on", False),
        ("", False),
    ],
...
def test_actor_probe_is_inert_when_disabled(monkeypatch):
    ...
    for name in ("_log_npu_mem", "_log_npu_expandable", "print_memory"):
        def _tripwire(*args, _name=name, **kwargs):
            del args, kwargs
            pytest.fail(f"{_name} ran while disabled")

        monkeypatch.setattr(train_actor, name, _tripwire)
...
def test_actor_probe_reports_device_and_host_together(monkeypatch):
    """All three readings are needed to attribute the residue."""
...
def test_actor_probe_survives_a_failing_reading(monkeypatch):
    ...
    monkeypatch.setattr(
        train_actor,
        "_log_npu_mem",
        lambda tag: (_ for _ in ()).throw(RuntimeError("boom")),
    )
...
def test_group_probe_fans_out_to_every_rank(monkeypatch):
    ...
    assert group.probe_memory("after onload_kv") == [
        (0, "after onload_kv"),
        (1, "after onload_kv"),
    ]
...
def test_startup_hand_off_is_probed_too():
    """The pre-loop weight sync trades memory like a rollout step."""
