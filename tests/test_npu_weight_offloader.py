...
@pytest.mark.parametrize(
    "requested, expected", [("auto", "storage-resize"), ("tms", "tms"), ("storage-resize", "storage-resize")]
)
...
@pytest.mark.unit
def test_tms_warns_when_the_actors_ask_for_expandable_segments(resolve, monkeypatch, caplog):
    """The actors take the allocator setting from --train-env-vars, not from this process."""
    resolver, _ = resolve
    monkeypatch.delenv("PYTORCH_NPU_ALLOC_CONF", raising=False)
    args = types.SimpleNamespace(
        npu_offload_backend="tms",
        train_env_vars={"PYTORCH_NPU_ALLOC_CONF": "expandable_segments:True"},
    )

    with caplog.at_level(logging.WARNING):
        assert resolver(args) == "tms"
