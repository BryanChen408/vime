"""GDN parameter layout conversion, shared by the mbridge converter and the model.
...
A) in_proj 融合/拆分(4 个,逐字 vendored from Megatron-Bridge,零改动):
  ...
  Source: megatron.bridge.models.conversion.param_mapping.
...
def merge_gdn_linear_weights(
    provider,  # TransformerConfig; untyped so importing it does not pull in transformer_engine
    qkvz: torch.Tensor,
    ba: torch.Tensor,
    tp_size: int = 1,
) -> torch.Tensor:
...
def _gdn_dims(config, tp_size: int) -> Tuple[int, int]:
    """Total q/k and v widths, validated against the TP size."""
    ...
    assert qk_dim % tp_size == 0 and v_dim % tp_size == 0, (
        f"GDN dims qk_dim={qk_dim}, v_dim={v_dim} must be divisible by tp_size={tp_size}"
    )
    return qk_dim, v_dim
...
def interleave_gdn_conv1d(conv: torch.Tensor, config, tp_size: int) -> torch.Tensor:
    ...
    """
    qk_dim, v_dim = _gdn_dims(config, tp_size)
    kernel = conv.shape[-1]
    segments = torch.split(conv, [qk_dim, qk_dim, v_dim], dim=0)
    segments = [w.reshape(tp_size, -1, 1, kernel) for w in segments]
    return torch.cat(segments, dim=1).reshape(-1, 1, kernel).contiguous()
...
def deinterleave_gdn_conv1d(conv: torch.Tensor, config, tp_size: int) -> torch.Tensor:
    ...
    conv = conv.reshape(tp_size, -1, 1, kernel)
    segments = torch.split(conv, [qk_local, qk_local, v_local], dim=1)
    segments = [w.reshape(-1, 1, kernel) for w in segments]
